...
export function isEmpty(obj = {}) {
  ...
  export function filterRows(rows, filters, globalFilter) {
    if (isEmpty(filters) && isNil(globalFilter)) return rows
  
    return rows.filter((row) => {
      const matchesFilters = Object.keys(filters).every((accessor) => {
        const value = row[accessor]
        const searchValue = filters[accessor]
      ...
      const matchesGlobalFilter = isString(globalFilter)
        ? Object.values(row)
            .map((value) => toLower(value))
            .some((value) => value.includes(toLower(globalFilter)))
        : true
  
      return matchesFilters && matchesGlobalFilter
    })
  }
